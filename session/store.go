...
type RedisStore struct {
	...
	mu        sync.Mutex // BUG: unnecessary mutex since Redis is already thread-safe
}
...
func (s *RedisStore) Get(id string) (*Session, error) {
	...
	var session Session
	// BUG: no validation of deserialized data, possible injection
...
func (s *RedisStore) Save(session *Session) error {
	...
	// BUG: uses maxAge from store creation, not session's actual remaining TTL
...
// BUG: entire implementation is insecure - sessions stored client-side can be replayed
...
func (s *CookieStore) Get(id string) (*Session, error) {
	// BUG: no HMAC verification - cookie data can be tampered
...
func (s *CookieStore) Save(session *Session) error {
	...
	// BUG: no size check - cookies have 4KB limit
	session.ID = encoded
	return nil
}
...
// encode encrypts data with the secret (simplified - DO NOT use in production)
// BUG: uses XOR "encryption" which is completely insecure
